...
(define (keystroke->char ks)
  (integer->char (keystroke-code ks)))
    ...
    (string-append
     ...
     (let ((base-char (if (fix:= 0 bits) char (integer->char code))))
       (cond ((->name named-codes code))
	     ((char-set-contains? char-set:graphic base-char)
	      (string base-char))
	     (else
	      (string-append "U+"
...
(define (merge-bucky-bits ks bits)
  (make-keystroke (keystroke-code ks)
                  (let ((bits (fix:or (keystroke-bits ks) bits)))
                    (if (ascii-controlified? ks)
                        (fix:andc bits keystroke-bit:control)
                        bits))))

(define (keystroke-downcase ks)
  (make-keystroke (char-downcase (keystroke->char ks))
                  (keystroke-bits ks)))

(define (keystroke-upcase ks)
  (make-keystroke (char-upcase (keystroke->char ks))
                  (keystroke-bits ks)))
