...

(define* (insert-char char (point (current-point)))
  (group-insert-char! (mark-group point) (mark-index point) char))

(define* (insert-chars char n (point (current-point)))
  (group-insert-chars! (mark-group point) (mark-index point) char n))

(define* (insert-newline (point (current-point)))
  (group-insert-char! (mark-group point) (mark-index point) #\newline))

(define* (insert-newlines n (point (current-point)))
  (group-insert-chars! (mark-group point) (mark-index point) #\newline n))

(define* (guarantee-newline (point (current-point)))
  (if (not (line-start? point))
      (insert-newline point)))

(define* (guarantee-newlines n (point (current-point)))
...
(define* (delete-right-char (point (current-point)))
  (let ((group (mark-group point))
        (index (mark-index point)))
...

(define* (insert object (point (current-point)))
  (insert-string (write-to-string object) point))

(define* (insert-string string (point (current-point)))
  (group-insert-string! (mark-group point) (mark-index point) string))

(define* (insert-substring string start end (point (current-point)))
  (group-insert-substring! (mark-group point) (mark-index point)
                           string start end))

(define* (insert-string-pad-left string n-columns (char #\space) (point (current-point)))
...
(define* (insert-substring-pad-left string start end n-columns
                                    (char #\space) (point (current-point)))
  (let ((group (mark-group point))
        (index (mark-index point))
...
(define* (insert-string-pad-right string n-columns
                                  (char #\space) (point (current-point)))
...
(define* (insert-substring-pad-right string start end n-columns
  ...
  (let ((length (fix:- end start)))
    (let ((group (mark-group point))
	  ...
	  (group-insert-substring! group index string start end)))))

(define* (insert-region start end (point (current-point)))
...
(define* (extract-string mark (point (current-point)))
...
(define* (delete-string mark (point (current-point)))
  (let ((group  (mark-group mark))
        (index1 (mark-index mark))
        (index2 (mark-index point)))
    (if (not (eq? group (mark-group point)))
...
(define* (extract-and-delete-string mark (point (current-point)))
  (let ((group  (mark-group mark))
        (index1 (mark-index mark))
        (index2 (mark-index point)))
    (if (not (eq? group (mark-group point)))
...
(define (find-previous-specific-property-change start end key)
  (let ((index
    ...
    (and index
	 (make-mark (mark-group start) index))))

(define* (specific-property-region mark key (predicate '()))
  (let ((default (list 'DEFAULT))
	(predicate
	 (if (or (null? predicate) (not predicate))
	     (lambda (x y) (eq? x y))
	     predicate)))
